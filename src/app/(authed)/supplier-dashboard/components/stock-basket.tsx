...
import { Loader2, Plus, Trash2, AlertTriangle, Package, CalendarIcon } from 'lucide-react';
...
export default function StockBasket() {
    ...
    const stockQuery = useMemo(() => {
        if (!user) return null;
        return query(stockCollection, where("supplierId", "==", user.uid), orderBy("expiryDate", "asc"));
